...
const Gallery: React.FC<GalleryProps> = () => {
    ...
    useEffect(() => {
        ...
        const transformWorks = (workObj: Project[]) => {
            const loadedWorks = [];
            for (const key in workObj) {
                loadedWorks.push({
                    id: key,
                    webSite: workObj[key].webSite,
                    fileUrl: workObj[key].fileUrl,
                });
            }
            setProjects(loadedWorks);
        };
    ...
    return (
        <section className="my-4 max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 my-8 max-w-5xl mx-auto" style={{ 'minHeight': '300px' }}>
                {projects.map(proj => (
                    <div className="relative shadow-sm md:shadow-2xl" key={proj.id}>
                        <img
                        ...
                        <div
                            className="absolute top-0 left-0 rounded w-full h-full grid place-items-end hover:opacity-100 "
                        >
                            <svg width="56px" height="56px" viewBox="0 0 256 256" className="shadow-lg" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid">
                                <g>
